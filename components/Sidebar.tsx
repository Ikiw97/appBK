    ...
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-screen bg-slate-900 text-slate-300 shadow-xl transition-all duration-300 z-50 flex flex-col ${isCollapsed ? 'w-20' : 'w-64'
          } ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 border-r border-slate-800`}
      >
        {/* Logo Section */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between h-16 md:h-20">
          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
            ...
            {!isCollapsed && (
              <div>
                <h2 className="text-base font-bold text-white">BK Dashboard</h2>
                <p className="text-xs text-slate-500">Asesmen Karir</p>
              </div>
            )}
          </div>
        </div>
